    ...
    try {
      ...
      const response = await fetch('https://backend.aisensy.com/campaign/t1/api/v2', {
        ...
        body: JSON.stringify({
          ...
          campaignName: 'Proposal-Sent',
          destination: phone,
          userName: parentsName,
          templateParams: [parentsName, visitDate, visitTime]
        })
      });
      ...
      if (onStatusUpdate) {
        onStatusUpdate('stage5_status', 'SENT');
      }

      console.log('Stage 5 (Proposal Sent) action completed');
      
    } catch (error) {
      console.error('Error updating Stage 5 status:', error);
      alert('Error updating Stage 5 status: ' + error.message);
    } finally {
      setIsLoading(false);
    }
  };
...
export default Stage5ActionButton;